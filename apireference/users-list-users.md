[Home](https://developer.webex.com/)/[Users](https://developer.webex.com/webex-contact-center/docs/api/v1/users)/List User(s)
...
**Operation Id:** getAllConfigWithMetaData\_4

**Description:**

Retrieve a list of User(s) in a given organization.
...
supervisorManagedAgentsOnlyboolean

If set to true, the API will return contact center enabled users based on the invoking supervisor user's user profile access rights to sites and teams.
...
buddyTeamAgentsOnlyboolean

If set to true, returns only users who are part of buddy teams without PBAC check.

default = false

userInQueuestring

Can be either assigned or unassigned. If passed, returns the users who are assigned or not assigned to an agent based queue managed by the supervisor.

example = "assigned"

queueIdstring

Contact Service Queue Id for which the list of assigned/unassigned agents needs to be fetched.
...
--url https://api.wxcc-us1.cisco.com/organization//v2/user \
